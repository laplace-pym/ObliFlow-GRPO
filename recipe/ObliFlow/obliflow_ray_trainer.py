...
"""ObliFlow-GRPO recipe trainer.

This trainer keeps the verl-agent PPO/RL plumbing and replaces state-distance
step rewards with post-hoc obligation-artifact flow rewards.
"""
...
from obli_flow import core_obliflow

from recipe.ObliFlow.rollout_loop import TrajectoryCollector
...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True, step_advantage_w=1.0, episode_advantage_w=0.0, gigpo_mode="mean_std_norm", **kwargs):
    ...
    Args:
        ...
        **kwargs: estimator-specific options such as ObliFlow alpha/beta/mode.
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
    elif adv_estimator == AdvantageEstimator.ObliFlow:
        advantages, returns = core_obliflow.compute_obliflow_outcome_advantage(
            token_level_rewards=data.batch['token_level_rewards'],
            flow_rewards=data.batch['flow_rewards'],
            cut_blames=data.batch['cut_blames'],
            response_mask=data.batch['response_mask'],
            index=data.non_tensor_batch['uid'],
            traj_index=data.non_tensor_batch['traj_uid'],
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    if self.config.algorithm.adv_estimator == AdvantageEstimator.GiGPO:
                        ...
                        metrics.update(flow_result.metrics)
