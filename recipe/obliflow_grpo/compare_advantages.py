"""
Compare GRPO / GiGPO / ObliFlow-GRPO advantage computations on the same task.
...
Usage:
    ...
    python -m recipe.obliflow_grpo.compare_advantages <path_to_pth_file>

    # Non-interactive — specify task by index or full UID
    python -m recipe.obliflow_grpo.compare_advantages <path_to_pth_file> 0
    python -m recipe.obliflow_grpo.compare_advantages <path_to_pth_file> <uid>
...
from recipe.obliflow_grpo.core_graph import (
    ComplexGraph,
    visualize_complex_graph_hierarchical,
    compute_graph_path_returns,
    compute_obliflow_grpo_outcome_advantage,
)
...
def compute_obliflow_grpo(batch, Gs, gamma=0.2, normalize_distance=False, mode="mean_std_norm",
    ...
    advantages, returns = compute_obliflow_grpo_outcome_advantage(
        token_level_rewards=b.batch['token_level_rewards'],
        step_rewards=b.batch['step_rewards'],
        response_mask=b.batch['response_mask'],
        anchor_obs=b.non_tensor_batch['anchor_obs'],
...
def main():
    ...
    batches = {
        ...
        "obliflow_grpo": compute_obliflow_grpo(batch, Gs=Gs, gamma=0.2, normalize_distance=False),
    }
    ...
    print(f"  03_obliflow_grpo_full.pdf                      — ObliFlow-GRPO (all trajs)")
    print(f"  traj00-{n_trajs-1:02d}_*_{{grpo,gigpo,obliflow_grpo}}.pdf  — per-trajectory ({n_trajs} x 3)")
