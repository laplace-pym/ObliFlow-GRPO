...
"""
ObliFlow-GRPO Recipe: FSDP PPO Trainer with Ray-based single controller.
...
from recipe.obliflow_grpo.rollout_loop import TrajectoryCollector
from recipe.obliflow_grpo.utils import adjust_batch
...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True, step_advantage_w=1.0, episode_advantage_w=0.0, gigpo_mode="mean_std_norm", **kwargs):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
    elif adv_estimator == AdvantageEstimator.ObliFlowGRPO:
        advantages, returns = core_graph.compute_obliflow_grpo_outcome_advantage(
            token_level_rewards=data.batch['token_level_rewards'],
            step_rewards=data.batch['step_rewards'],
            response_mask=data.batch['response_mask'],
            anchor_obs=data.non_tensor_batch['anchor_obs'],
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        if self.config.algorithm.adv_estimator == AdvantageEstimator.GAE:
            ...
            AdvantageEstimator.ObliFlowGRPO,
        ]:
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    if self.config.algorithm.adv_estimator == AdvantageEstimator.GiGPO:
                    ...
                    elif self.config.algorithm.adv_estimator == AdvantageEstimator.ObliFlowGRPO:
                        obliflow_grpo_cfg = self.config.algorithm.get("obliflow_grpo", {})
                        Gs = core_graph.ComplexGraph.from_data(
                            gen_batch_output,
                            enable_similarity=obliflow_grpo_cfg.get("enable_similarity", False),
                            similarity_thresh=obliflow_grpo_cfg.get("similarity_thresh", 0.95),
                        )
                        ...
                        step_rewards_tensor = core_graph.compute_graph_path_returns(
                            ...
                            Gs=Gs,
                            gamma=self.config.algorithm.gamma,
                            normalize_distance=obliflow_grpo_cfg.get("normalize_distance", False),
                        )
                        batch.batch['step_rewards'] = step_rewards_tensor
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        batch = compute_advantage(
                            ...
                            step_advantage_w=self.config.algorithm.get("obliflow_grpo", {}).get("step_advantage_w", 1.0),
                            episode_advantage_w=self.config.algorithm.get("obliflow_grpo", {}).get("episode_advantage_w", 1.0),
                            gigpo_mode=self.config.algorithm.get("obliflow_grpo", {}).get("mode", "mean_std_norm"),
                            epsilon=1e-6,
                        )
